...
void GraphicalObject::createPipelines(VkDevice logicalDevice, VkRenderPass renderPass, VkExtent2D extent, std::vector<GraphicalObject*>& objects) {
	...
	for (size_t i = 0; i < objects.size(); i++) {
		...
		std::vector<VkPushConstantRange> pushConstantBindings = objects[i]->setPushConstantRanges();

		VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
		pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
		pipelineLayoutInfo.setLayoutCount = 0;
		pipelineLayoutInfo.pushConstantRangeCount = pushConstantBindings.size();
		pipelineLayoutInfo.pPushConstantRanges = pushConstantBindings.data();
...
void GraphicalObject::createDescriptorSetLayout(VkDevice logicalDevice) {
	std::vector<VkDescriptorSetLayoutBinding> layoutBindings = setDescriptorLayoutBindings();
