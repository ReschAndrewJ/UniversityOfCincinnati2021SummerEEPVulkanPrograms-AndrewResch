...
void App::setRoot(BaseObject* obj) {
	...
	rootObject->parentApp_shouldUpdateFunc = &objectsShouldUpdate;
...
void App::mainLoop() {
	while (!engine.windowShouldClose()) { 
		...
		if (updateExistingObjects) {
			engine.updateObjects(graphicsObjects);
...
void App::objectsShouldUpdate(void* app) {
	reinterpret_cast<App*>(app)->updateExistingObjects = true;
}
