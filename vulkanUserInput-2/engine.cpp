...
void engine::mainLoop() {
	while (!glfwWindowShouldClose(window.window)) {
		...
		for (std::map<int, Object>::iterator it = objectUpdateQueue.begin(); it != objectUpdateQueue.end(); ++it) {
			updateObject(it->second, it->first);
		}
		objectUpdateQueue.clear();

		std::sort(objectRemoveQueue.begin(), objectRemoveQueue.end());
		std::unique(objectRemoveQueue.begin(), objectRemoveQueue.end());
...
void engine::keyCallback(GLFWwindow* window, int key, int scanCode, int action, int mods) {
	engine* app = reinterpret_cast<engine*>(glfwGetWindowUserPointer(window));
	app->inputHandle.keyAction(key, action);
}
